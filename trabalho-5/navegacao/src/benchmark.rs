...
use std::time::Instant; // Usamos o rand do macroquad

/// Preenche o grid com obstáculos aleatórios
...
fn generate_agent_tasks(grid: &Grid, n: usize) -> Vec<((usize, usize), (usize, usize))> {
    ...
    for _ in 0..n {
        // Tenta encontrar um início e fim válidos
        ...
        for &density in &densities {
            for &n_agents in &agent_counts {
                ...
                for _ in 0..repetitions {
                    ...
                    if tasks.is_empty() {
                        continue;
                    } // Pula se não conseguiu gerar agentes

                    // 3. Medir o tempo
                    let start = Instant::now();
                    for (start_pos, end_pos) in &tasks {
                        // Executa o A* para cada tarefa
                        let _ = a_star_search(&grid, *start_pos, *end_pos);
                    }
