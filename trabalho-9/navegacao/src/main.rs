...
use observer::{RespawnHandler, AgentEvent}; // Adicionado AgentEvent
...
async fn main() {
    ...
    loop {
        ...
        // 1. Atualiza lógica interna dos agentes
        ...
        let agent_states: Vec<(usize, Vec2, f32, f32)> = agents
            ...
            .map(|a| (
                a.get_id(),
                a.get_pos(),
                a.get_physical_radius(),
                a.get_detection_radius(),
            ))
            .collect();

        // 2. Gera Comandos de Movimento com Verificação
        for agent in &agents {
            if let Some(target_pos) = agent.get_next_step_target() {
                ...
                // Verifica contra todos os outros
                for (other_id, other_pos, other_phys_r, _) in &agent_states {
                    if *other_id == id {
                        continue; // Não checa contra si mesmo
                    }
                    ...
                    if dist < sum_physical {
                        ...
                        agent.notify(AgentEvent::CollisionHit(*other_id));
                    } else if dist < sum_detection {
                        // Colisão Prevista (Antes de acontecer)
                        agent.notify(AgentEvent::ProximityAlert(*other_id));
                    }
                }
